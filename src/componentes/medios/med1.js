...
export class Medix extends Component {
    ...
    detalle(){
        window.location.href = '/detalle-reporte'+ "#" + this; 
    }
    render() {
        ...
        if(  this.props.estado === reporte.State)
         {
            return (
                ...
                <button className="totalerroresalert btn btn-primary pt-0 pb-0" onClick={this.detalle.bind(reporte.LocalMediaID)}>Ver</button>
                </div></div></div></div>
              )
         }
         else{        
